# API仕様書

## API一覧

|No.|API機能No.|API名|機能概要|
|---|---|---|---|
|0|ShunNaviAPI-000|献立|緯度経度情報に応じた献立の取得|
|1|ShunNaviAPI-001|食材|指定した食材についてのデータを返す|

<br>

----

## ShunNaviAPI-000

|API機能No|ShunNaviAPI-000|
|---|---|
|API名|献立の取得|
|更新日/更新者||
|概要|クライアントの位置情報をサーバーに送信しその位置情報から取得した県の食材を使った旬の献立を取得する|
|||
<br>

|アクセスURL||
|---|---|
|旬の献立 | /api/v1/menu/ |
|カテゴリリスト | /api/v1/menu/category_list/ |
|カテゴリ内ランキング | /api/v1/menu/ranking/:id |
|||

<br>

|入力データ| | | | | | 
|---|---|---|---|---|---|
| *アクセス方法* | *GET* | | | | |
| *形式* | *JSON* | | | | |
|**JSON Key** | **型**| **サイズ** | **必須** | **繰返** | **値の説明** |
|longitude|数値|||| 言葉の通り経度 |
|latitude|数値|||| 言葉の通り緯度 |

### 楽天レシピAPI 対応，使用するデータ
| 元の値 | 変換後 | 備考 |
|---|---|---|
| result | recipeList | |
| - recipeTitle | - name | |
| - mediumImageUrl | - picture | |
| - recipeDescription| - recipeDescription | |
| - recipeMaterial | - foods | 配列 |
| - recipeCost | - cost | 文字列をそのまま |
| - recipeIndication | - time | 調理時間 |


<br>

|出力データ| | | | | | 
|---|---|---|---|---|---|
|**JSON Key** | **型**| **最大サイズ** | **必須** | **繰返** | **値の説明** |
| recipiList | 配列 | | 〇 | | レシピの一覧 |
| - id | 数値 |  | 〇 |  | 管理用 |
| - name | 文字列 |  | 〇 |  | 料理名 |
| - picture | 文字列 | | 〇 | | 料理の画像のリンク |
| - foods | 配列 | | 〇 | | 使われている食材の一覧 |
| -- food | 連想配列 | | 〇 | | 食材 |
| --- id | 数値 | | 〇 | | 食材id |
| --- name | 文字列 | | 〇 | | 食材名 |
| --- syun | ブーリアン | | | | 今が旬か否か |
| --- pref_id | 数値 | | 〇 | | 県id |
| - pref_id | 数値 | | 〇 | | 主にどこの県の料理か |
| - prefecture | 文字列 | | 〇 | | 主にどこの県の料理か |
| - howto | 文字列 | | 〇 | | 作り方（または楽天レシピのURL） |
| - post | ブーリアン | | 〇 | | 投稿されたものならTrue |
| - cost | 文字列 | | | | 予算の目安 |
| - time | 文字列 | | | | 調理時間の目安 |

|ステータスコード|説明|
|---|---|
|  | 位置情報がおかしい，県名が取得できなかった(null)  |

<br>

---

## ShunNaviAPI-001

|API機能No|ShunNaviAPI-001|
|---|---|
|API名|食品の詳細取得|
|更新日/更新者||
|概要|食材の産地，特徴，値段，栄養価，価格推移についてを取得する|

<br>

|アクセスURL||
|---|---|
|食品データ| /api/v1/food/:id |

<br>

|入力データ| | | | | | 
|---|---|---|---|---|---|
| *アクセス方法* | *GET*| | | | |
| *形式* | *JSON* | | | | |
|**JSON Key** | **型**| **サイズ** | **必須** | **繰返** | **値の説明** |
|foods| 配列 |  | 〇 |  |  |
|- id| 数値 |  | 〇 |  | 管理用 |

<br>

|出力データ| | | | | | 
|---|---|---|---|---|---|
|**JSON Key** | **型**| **最大サイズ** | **必須** | **繰返** | **値の説明** |
| food_info | オブジェクト | | 〇 | | 食材についての詳細情報 |
| - id | 数値 | | | | |
| - name | 文字列 | | | | |
| - basefood | 文字列 | | | | |
| - picture | 文字列 | | | | 食材の画像のリンク |
| - months | 配列 | | | | | 
| - pref_id | 数値 | | | | 産地のid |
|---|---|---|---|---|---|
| prices | オブジェクト | | | | 価格遷移グラフの作成用 |
| key | 文字列 | | | | 産地 |
| - HighPrice | 配列 | | | | 高値 |
| - MediumPrice | 配列 | | | | 中値 | 
|---|---|---|---|---|---|
| nutritional | オブジェクト |||| 含まれている栄養分 |
| - food | 文字列 |||| 食材名 | 
| - info | 配列 | |||| 食材の栄養素一覧について |
| -- name | 文字列 |||| 栄養素名 |
| -- value | 文字列 |||| 量 |
| -- unit | 文字列 |||| 単位 |


#### レスポンス例
/api/v1/food/112
```json
{
  "food_info": {
    "id": 112,
    "name": "バナナ",
    "base_food": null,
    "picture": null,
    "months": "",
    "pref_id": 0,
    "post": 0
  },
  "prices": {
    "ｴｸｱﾄﾞﾙ共和国": {
    "HighPrice": [
      "269",
      "269",
      "-",
      "269",
      "269",
      "269",
      "269",
      "269"
    ],
    "MediumPrice": [
      "-",
      "-",
      "215",
      "205",
      "205",
      "215",
      "-",
      "-"
    ]
  },
  "ﾌｨﾘﾋﾟﾝ共和国": {
    "HighPrice": [
      "286",
      "307",
      "735",
      "286",
      "735",
      "286",
      "286",
      "286"
    ],
    "MediumPrice": [
      "200",
      "200",
      "215",
      "215",
      "215",
      "215",
      "215",
      "215"
    ]
  }
  },
  "nutritional": {
    "food": "果実類/バナナ/生",
    "info": [
      {
      "name": "エネルギー",
      "value": "86",
      "unit": "kcal"
      },
      {
      "name": "たんぱく質",
      "value": "1.1",
      "unit": "g"
      },
      {
      "name": "脂質",
      "value": "0.2",
      "unit": "g"
      },
      {
      "name": "炭水化物",
      "value": "22.5",
      "unit": "g"
      },
      {
      "name": "ナトリウム",
      "value": "",
      "unit": "mg"
      },
      {
      "name": "カリウム",
      "value": "360",
      "unit": "mg"
      },
      {
      "name": "カルシウム",
      "value": "6",
      "unit": "mg"
      },
      {
      "name": "マグネシウム",
      "value": "32",
      "unit": "mg"
      },
      {
      "name": "コレステロール",
      "value": "0",
      "unit": "mg"
      },
      {
      "name": "食塩相当量",
      "value": "0",
      "unit": "g"
      }
    ]
  }
}
```

|ステータスコード|説明|
|---|---|
|||

<br>

---

